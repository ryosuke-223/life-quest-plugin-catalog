...
def map_style():
    return {
        "markerSVG": '<svg viewBox="0 0 10 10"><path d="M0 0h10v10H0z"/></svg>',
        "markerColors": {
            "unvisited": "#112233",
            "estimated": "#44556677",
            "confirmed": "#AABBCC",
...
class ValidateCatalogTests(unittest.TestCase):
    ...
    def test_accepts_map_style_with_static_svg_and_status_colors(self):
        plugin = manifest()
        plugin["mapStyle"] = map_style()
        validate_plugin(Path("test.json"), plugin)

    def test_rejects_unsafe_map_style_svg_and_invalid_color(self):
        plugin = manifest()
        plugin["mapStyle"] = map_style()
        plugin["mapStyle"]["markerSVG"] = '<svg viewBox="0 0 10 10"><script>alert(1)</script></svg>'
        with self.assertRaisesRegex(ValueError, "mapStyle.markerSVG"):
            validate_plugin(Path("test.json"), plugin)

        plugin = manifest()
        plugin["mapStyle"] = map_style()
        plugin["mapStyle"]["markerColors"]["confirmed"] = "red"
        with self.assertRaisesRegex(ValueError, "mapStyle.markerColors.confirmed"):
            validate_plugin(Path("test.json"), plugin)

        plugin = manifest()
        plugin["mapStyle"] = map_style()
        plugin["mapStyle"]["markerSVG"] = '<svg viewBox="0 0 10 10"><path transform="scale(2)" d="M0 0h10v10H0z"/></svg>'
        with self.assertRaisesRegex(ValueError, "mapStyle.markerSVG"):
            validate_plugin(Path("test.json"), plugin)

    def test_rejects_invalid_group_marker_svg(self):
        plugin = manifest()
        plugin["groups"] = [{
            "id": "cultural",
            "title": "文化遺産",
            "mapMarkerSVG": '<svg viewBox="0 0 10 10"><image href="https://example.com/x"/></svg>',
        }]
        with self.assertRaisesRegex(ValueError, "cultural.mapMarkerSVG"):
            validate_plugin(Path("test.json"), plugin)

    def test_shipped_manifests_declare_plugin_owned_map_style(self):
        plugin_directory = Path(__file__).resolve().parents[1] / "plugins"
        expected_fields = {"markerSVG", "markerColors", "areaFillColors", "areaStrokeColors"}
    ...
    def test_airport_generators_declare_year_charts_and_compact_item_details(self):
        ...
        self.assertEqual(set(japan["mapStyle"]), {
            "markerSVG", "markerColors", "areaFillColors", "areaStrokeColors"
        })
        self.assertEqual(set(world["mapStyle"]), set(japan["mapStyle"]))
        validate_plugin(Path("japan.json"), japan)
        validate_plugin(Path("world.json"), world)


class WorldHeritageDataTests(unittest.TestCase):
    def test_world_heritage_manifest_declares_all_time_dashboard_and_marker_styles(self):
        ...
        self.assertNotIn("mapMarkerSymbolName", {key for group in plugin["groups"] for key in group})
        self.assertEqual(set(plugin["mapStyle"]), {
            "markerSVG", "markerColors", "areaFillColors", "areaStrokeColors"
        })
        self.assertIn({"type": "metricCards", "metrics": [
            "visitedCount", "completionRate", "remainingCount", "unlockedAchievements"]},
            plugin["visualizations"])
        self.assertIn({"type": "lineChart", "metric": "visitedCount", "aggregation": "cumulativeCount"},
                      plugin["visualizations"])
        self.assertIn({"type": "barChart", "metric": "visitedCount", "aggregation": "annualVisitedItemCount"},
                      plugin["visualizations"])
        self.assertIn({"type": "statusMap", "cluster": True}, plugin["visualizations"])
        self.assertEqual(next(item for item in plugin["visualizations"] if item["type"] == "itemList")["sort"],
                         "title")
